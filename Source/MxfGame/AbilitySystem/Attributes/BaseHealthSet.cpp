...
bool UBaseHealthSet::PreGameplayEffectExecute(FGameplayEffectModCallbackData &Data)
{
	...
	if (Data.EvaluatedData.Attribute == GetDamageAttribute())
	{
		if (Data.EvaluatedData.Magnitude > 0.0f)
		{
			...
			if (Data.Target.HasMatchingGameplayTag(BaseGameplayTags::Cheat_GodMode) && !bIsDamageFromSelfDestruct)
			{
				// Do not take away any health.
				Data.EvaluatedData.Magnitude = 0.0f;
				return false;
...
void UBaseHealthSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	if (!bIsDamageFromSelfDestruct &&
		(Data.Target.HasMatchingGameplayTag(BaseGameplayTags::Cheat_GodMode) || Data.Target.HasMatchingGameplayTag(BaseGameplayTags::Cheat_UnlimitedHealth) ))
	{
		MinimumHealth = 1.0f;
	}
#endif // #if !UE_BUILD_SHIPPING
...
void UBaseHealthSet::PreAttributeBaseChange(const FGameplayAttribute& Attribute, float& NewValue) const
{
...
void UBaseHealthSet::PreAttributeChange(const FGameplayAttribute& Attribute, float& NewValue)
{
...
void UBaseHealthSet::PostAttributeChange(const FGameplayAttribute& Attribute, float OldValue, float NewValue)
{
...
void UBaseHealthSet::ClampAttribute(const FGameplayAttribute& Attribute, float& NewValue) const
{
