	...
	const UBaseInputSet* InputSet)
{
    ...
    // 用于缓存已找到的对象引用，避免在同一个 InputSet 中多次调用 GetComponentByClass
    TMap<TSubclassOf<UObject>, UObject*> ResolvedObjects;

    for (const FBaseInputSet_InputMapping& Mapping : InputSet->InputMappings)
    {
	    if (!Mapping.InputTag.IsValid() || Mapping.FuncClass == nullptr)
	    {
	    	continue;
	    }
    	...
    	for (const FBaseInputSet_InputMapping& InputMapping : InputSet->InputMappings)
    	{
    		...
    		else
    		{
    			...
    			{
    				TargetObject = Cast<AActor>(Object)->GetComponentByClass(*InputMapping.FuncClass);
    			}
